...
public class Filter {
    ...
    /**
     * 
     * Maximum cutoff frequency is specified as FCmax = 2.6e-5/C = 2.6e-5/2200e-12 =
     * 11818.
     * <P>
     * Measurements indicate a cutoff frequency range of approximately 220Hz - 18kHz
     * on a MOS6581 fitted with 470pF capacitors. The function mapping FC to cutoff
     * frequency has the shape of the tanh function, with a discontinuity at FCHI =
     * 0x80. In contrast, the MOS8580 almost perfectly corresponds with the
     * specification of a linear mapping from 30Hz to 12kHz.
     * <P>
     * The mappings have been measured by feeding the SID with an external signal
     * since the chip itself is incapable of generating waveforms of higher
     * fundamental frequency than 4kHz. It is best to use the bandpass output at
     * full resonance to pick out the cutoff frequency at any given FC setting.
     * <P>
     * The mapping function is specified with spline interpolation points and the
     * function values are retrieved via table lookup.
     * <P>
     * NB! Cutoff frequency characteristics may vary, we have modeled two particular
     * Commodore 64s.
     */
     ...
     * @param ext_in
     */
    public void clock(int /* sound_sample */ voice1, int /* sound_sample */ voice2, int /* sound_sample */ voice3,
        ...
        if (ANTTI_LANKILA_PATCH) {
            int /* sound_sample */ Vi_peak_bp = ((Vlp * DHlp + Vbp * DHbp + Vhp * DHhp) >> 8) + Vi;
            if (Vi_peak_bp < DHthreshold)
                Vi_peak_bp = DHthreshold;
            int /* sound_sample */ Vi_peak_lp = ((Vlp * DLlp + Vbp * DLbp + Vhp * DLhp) >> 8) + Vi;
            if (Vi_peak_lp < DLthreshold)
                Vi_peak_lp = DLthreshold;
            int /* sound_sample */ w0_eff_bp = w0 + w0 * ((Vi_peak_bp - DHthreshold) >> 4) / DHsteepness;
            int /* sound_sample */ w0_eff_lp = w0 + w0 * ((Vi_peak_lp - DLthreshold) >> 4) / DLsteepness;
     ...
     * @param ext_in
     */
    public void clock(int /* cycle_count */ delta_t, int /* sound_sample */ voice1, int /* sound_sample */ voice2,
    ...
    void set_distortion_properties(int Lthreshold, int Lsteepness, int Llp, int Lbp, int Lhp, int Hthreshold,
            int Hsteepness, int Hlp, int Hbp, int Hhp) {
     ...
     * @param coeff
     */
    protected void cubic_coefficients(double x1, double y1, double x2, double y2, double k1, double k2,
            Coefficients coeff) {
     ...
     * @param res
     */
    protected void interpolate_brute_force(double x1, double y1, double x2, double y2, double k1, double k2,
     ...
     * @param res
     */
    protected void interpolate_forward_difference(double x1, double y1, double x2, double y2, double k1, double k2,
            PointPlotter plotter, double res) {
        Coefficients coeff = new Coefficients();
        cubic_coefficients(x1, y1, x2, y2, k1, k2, coeff);
     ...
     * interpolated. If extra control points are not desirable, the end points can
     * simply be repeated to ensure interpolation. Note also that points of
     ...
     * @param res
     */
    public void interpolate(int[] /* fc_point */ f0_base[], int p0, int pn, PointPlotter plotter, double res) {
        double k1, k2;
        ...
        for (; p2 != pn; ++p0, ++p1, ++p2, ++p3) {
            ...
            if (SPLINE_BRUTE_FORCE) {
                interpolate_brute_force(x(f0_base, p1), y(f0_base, p1), x(f0_base, p2), y(f0_base, p2), k1, k2, plotter,
                        res);
            } else {
                interpolate_forward_difference(x(f0_base, p1), y(f0_base, p1), x(f0_base, p2), y(f0_base, p2), k1, k2,
                        plotter, res);
            }
        }
    }
