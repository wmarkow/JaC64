 ...
 * @version 1.0
 * 
 *          GCR Coding Table Nybble GCR 0 0 0000 01010 a 10 1 1 0001 01011 b 11
 *          2 2 0010 10010 12 18 3 3 0011 10011 13 19 4 4 0100 01110 e 14 5 5
 *          0101 01111 f 15 6 6 0110 10110 16 22 7 7 0111 10111 17 23 8 8 1000
 *          01001 9 9 9 9 1001 11001 19 25 a 10 1010 11010 1a 26 b 11 1011 11011
 *          1b 27 c 12 1100 01101 d 13 d 13 1101 11101 1d 29 e 14 1110 11110 1e
 *          30 f 15 1111 10101 15 21
 * 
 */
...
public class C1541Chips extends ExtChip implements DiskListener {
    ...
    // (or 4 bits to 5)
    public static final int[] GCR = new int[] { 0x0a, 0x0b, 0x12, 0x13, 0x0e, 0x0f, 0x16, 0x17, 0x09, 0x19, 0x1a, 0x1b,
            0x0d, 0x1d, 0x1e, 0x15 };

    // 5 bits > 4 bits (0xff => invalid)
    public static final int[] GCR_REV = new int[] { 0xff, 0xff, 0xff, 0xff, // 0 - 3invalid...
    ...
    public final int performRead(int address, long cycles) {
        ...
        case 0x1800: // VIA1 PB - IEEE Serial Bus / IEC
            return (via1PB & 0x1a | ((iecLines & cia2.getIecLines()) >> 7) & 0x01 // DATA
    ...
    public final void performWrite(int address, int data, long cycles) {
        if (DEBUG_WRITE) {
            log("Writing to " + Integer.toHexString(address) + " = " + Integer.toHexString(data) + " pc = "
        ...
        case 0x180e:
            if ((data & 0x80) == 0x80)
                ...
                via1IEnable &= ~data;
            if (DEBUG_WRITE)
                System.out.println("Wrote IE CIA1: " + via1IEnable);
    ...
    private void writeByte(int data) {
        currentByte = data;
        log("CPU Writes byte: " + Integer.toString(data, 16) + " at " + Integer.toString(cpu.getPc(), 16));
        // forward();
    }

    private void finishWrite() {
        if (DEBUG) {
            log("Written bytes: " + bytesWritten + " at " + track + "," + sector + "  " + sectorPos);
        }
        if (bytesWritten > 0)
            convertGCRSector();
        bytesWritten = 0;
        ...
        if (via == 1) {
            if ((via1IFlag & via1IEnable) == 0) {
                if (DEBUG_IRQ && (via1IFlag & 0x80) != 0) {
                    System.out.println("C1541: ** IRQ/IEC ** Clearing IRQ CIA 1: " + reason);
                }
                via1IFlag &= 0x7f;
                clearIRQ(1);
            } else {
                if (DEBUG_IRQ && (via1IFlag & 0x80) == 0) {
                    System.out.println("C1541: ** IRQ/IEC ** Setting IRQ CIA 1: " + reason + " iflag: "
            ...
            if ((via2IFlag & via2IEnable) == 0) {
                if (DEBUG_IRQ && (via2IFlag & 0x80) != 0) {
                    System.out.println("C1541: ** IRQ ** Clearing IRQ CIA 2: " + reason);
                }
                via2IFlag &= 0x7f;
                clearIRQ(2);
            } else {
                if (DEBUG_IRQ && (via2IFlag & 0x80) == 0) {
                    System.out.println("C1541: ** IRQ ** Setting IRQ CIA 2: " + reason + " iflag: "
    ...
    public void updateIECLines() {
        ...
        iecLines = (data << 6) & ((~data ^ cia2.getIecLines()) << 3) & 0x80 | (data << 3) & 0x40;
    }
    ...
    public void atnChanged(boolean hi) {
        if (DEBUG_IEC) {
            System.out.println("C1541: *** IEC ATN changed detected!!! Enable: " + Integer.toHexString(via1IEnable));
        }
        if (hi) {
            via1IFlag |= 0x02;
    ...
    private int readByte() {
        ...
        if (DEBUG_GCR) {
            System.out.println("C1541: Read byte from: " + sectorPos + " => " + Integer.toString(data, 16) + " "
                    + i2c(data) + " " + cpu.cycles + " pc = " + Integer.toString(cpu.getPc(), 16));
        }
        // forward();
        return data;
    }
    ...
    private void forward() {
        if (diskModeWrite && (via2CA == 0xff) && currentByte != -1 && sectorPos >= 0) {
            bytesWritten++;
            if (DEBUG) {
                System.out.println("#### Write byte: " + Integer.toString(currentByte, 16) + " ("
                        + Integer.toString(gcrSector[sectorPos], 16) + ")" + (char) currentByte + "  y=" + cpu.getY()
                        + " written: " + bytesWritten + " SectorPos: " + sectorPos + " " + cpu.cycles);
            }
    ...
    private void updateHTrack() {
        if (track != hTrack >> 1) {
            ...
            currentTrackSize = C64Reader.getSectorCount(track);
            System.out.println(
                    "1541: New Track " + track + " reading: " + track + ", " + sector + " s/t: " + currentTrackSize);
            readGCRSector(track, sector);
        }
    }
    ...
    public static int makeGCR(int[] gcrBuf, int pos, int b1, int b2, int b3, int b4) {
        ...
        if (DEBUG_GCR) {
            System.out.print("GCR 4 => 5: " + i2c(b1) + i2c(b2) + i2c(b3) + i2c(b4) + ", " + b1 + ", " + b2 + ", " + b3
                    + ", " + b4);
        }
    ...
    private void readGCRSector(int track, int sector) {
        ...
        if (gcrCacheSector[track][sector] != null) {
            if (DEBUG_GCR)
                log("Using GCR Sector cache for " + track + "," + sector);
        ...
        if (DEBUG_GCR)
            log("Reading and GCR:ing sector: " + track + ", " + sector);
        ...
        makeGCR(gcrSector, pos, 0x08, sector ^ track ^ diskID1 ^ diskID2, sector, track);
        pos += 5;
        ...
        cSum = 0x07;
        cSum ^= makeGCR(gcrSector, pos, 0x07, sectorBuf[0] & 0xff, sectorBuf[1] & 0xff, sectorBuf[2] & 0xff);
        pos += 5;

        // pos = 26
        for (int i = 3, n = 255; i < n; i += 4) {
            cSum ^= makeGCR(gcrSector, pos, sectorBuf[i] & 0xff, sectorBuf[i + 1] & 0xff, sectorBuf[i + 2] & 0xff,
                    sectorBuf[i + 3] & 0xff);
            pos += 5;
        }
    ...
    private void convertFromGCR(int[] buff, int pos, int posW) {
        long data = convertFromGCR(gcrWriteSector[posW], gcrWriteSector[posW + 1], gcrWriteSector[posW + 2],
                gcrWriteSector[posW + 3], gcrWriteSector[posW + 4]);
