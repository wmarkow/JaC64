...
/**
 * class C1541 - simple SerialBus and C1541 emulation. - will not support
 * everything but the usual simplest CBM dos stuff. No fastloading, etc. Might
 * add full 1541 emulation later with CPU + IO chip emulation.
 *
 * Created: Tue Mar 28 22:06:31 2006
 *
 * @author Joakim Eriksson, Dreamfabric.com joakime@sics.se
 * @version 1.0
 */
public class C1541 extends Observable {
    ...
    private boolean writeByte(int data, long cycles, boolean timeout) {
        ...
        case WRITE_END:
            clockLo();
            if (dataOut) {
                System.out.println("Ack: " + Integer.toString(memory[0xa4], 16));
                wBytePos++;
    ...
    private void initWrite(long cycles) {
        ...
        if (floppyMode == LOAD_FILE) {
            ...
            if ((filename = reader.readFile(filename, -1, out)) != null) {
                ...
                System.out.println("C1541 have " + bytesToWrite.length + " bytes to write");
            ...
            if (dev == 31) {
                ...
                if (floppyMode == LOGICAL_CHANNEL) {
                    ...
                    if ((tmpFilename = reader.readFile(tmpFilename, -1, out)) != null) {
                        ...
                        System.out.println("Setting channel " + floppyChannel + " to " + tmpFilename + " size: "
