...
public class SIDVoice6581 extends SIDVoice {
    ...
    public static final String[] WAVE = new String[] { "NONE", "TRIANGLE", "SAW", "TRI_SAW", "PULSE", "TRI_PULSE",
            "SAW_PULSE", "TRI_SAW_PULSE", "NOISE", "NOISE_T", "NOISE_S", "NOISE_TS", "NOISE_P", "NOISE_TP", "NOISE_SP",
            "NOISE_TSP" };
    ...
    long attackTime[] = { 2000, 8000, 16000, 24000, 38000, 56000, 68000, 80000, 100000, 250000, 500000, 800000, 1000000,
            3000000, 5000000, 8000000 };
    ...
    long decayExp[] = { 4216751472L, 4275278366L, 4285111523L, 4288394265L, 4290814737L, 4292149050L, 4292646253L,
            4292994329L, 4293388850L, 4294335848L, 4294651560L, 4294769958L, 4294809425L, 4294914671L, 4294935721L,
            4294947561L }; // was new long[attackTime.length];
    ...
    public void setControl(int data, long cycles) {
        wave = data >> 4;
        if (DEBUG) {
            System.out.println(getSIDNo() + "  Setting Wave: " + WAVE[wave] + " at " + cycles + "  lastGen: "
                    + lastCycles + " Gate: " + (data & 1));
        }
        boolean oldTest = testBit;
        testBit = (data & 0x08) != 0;
        if (oldTest && !testBit) {
            // When cleared -> reset sample play...
            // System.out.println(getSIDNo() + " Testbit cleared!!!>>> " + cycles + "
            // zeroed:" +
            // testZero);
            testZero = 0;
            nextSample = 0;
    ...
    public void soundOn(long cycles) {
        if (!soundOn) {
            ...
            if ((sr & 0xf) > (ad >> 4)) {
                ...
                if (DEBUG)
                    System.out.println(getSIDNo() + "  CTRATK_ADSR BUG Triggered for " + adsrBug + " samples"
                            + " level: " + (adsrLevel22 >> ADSR_BITS));
    ...
    public void setAD(int data, long cycles) {
        ...
        if (DEBUG) {
            System.out.println(getSIDNo() + "  Setting AD: " + memory[sidbase + 5] + " " + cycles + " diff "
                    + (cycles - lastAttackTime) + " ADSRLv: " + (adsrLevel22 >> ADSR_BITS));
        }
        ...
        if (adsrPhase != RELEASE) {
            if (currentDecayFactor > decayFactor) {
                adsrBug = (int) (0x8000 * SAMPLES_PER_MICRO);
                if (DEBUG) {
                    System.out.println(getSIDNo() + "  DEC_ADSR BUG Triggered for " + adsrBug + " samples" + " level: "
    ...
    public void setSR(int data, long cycles) {
        ...
        if (DEBUG) {
            System.out.println(getSIDNo() + "  Setting SR: " + Integer.toString(memory[sidbase + 6], 16) + " " + cycles
                    + " diff " + (cycles - lastAttackTime) + " ADSRLv: " + (adsrLevel22 >> ADSR_BITS));
        }
        ...
        if (adsrPhase == RELEASE) {
            if (currentDecayFactor > releaseFactor) {
                adsrBug = (int) (0x8000 * SAMPLES_PER_MICRO);
                if (DEBUG) {
                    System.out.println(getSIDNo() + "  REL_ADSR BUG Triggered for " + adsrBug + " samples" + " level: "
                            + (adsrLevel22 >> ADSR_BITS));
                }
            }
            currentDecayFactor = releaseFactor;
    ...
    public void updatePulseWidth(long cycles) {
        if ((wave & PULSE) != 0 && lastCycles > 0) {
            int pos = (int) ((cycles - lastCycles) / (SIDVoice6581.CYCLES_PER_SAMPLE + 10));
            pwidArr[pos % SIDVoice6581.GENLEN] = ((((memory[sidbase + 3] & 0xf) << 8) + memory[sidbase + 2]) * waveLen)
                    / 4095;
        } else {
            pwid = ((((memory[sidbase + 3] & 0xf) << 8) + memory[sidbase + 2]) * waveLen) / 4095;
        }
    }
    ...
    public void updateSound(long cycles) {
        ...
        // now???
        frq = (int) (0.5 + (1 << FRQ_BITS) * ((memory[sidbase + 1] << 8) + memory[sidbase]) * FRQCONV);
        trueFrq = (1 << FRQ_BITS) * ((memory[sidbase + 1] << 8) + memory[sidbase]) * FRQCONV;
        ...
        case TRIANGLE:
            if (ring) {
                boolean msb0 = false;
                for (int i = 0; i < GENLEN; i++) {
                    boolean msb = (msb0 = nextSample >= WAVELEN_HALF) ^ (next_nextSample >= WAVELEN_HALF);
        ...
        case SAW | TRIANGLE:
            if (!sync) {
                for (int i = 0; i < GENLEN; i++) {
                    intBuffer[bIndex++] = ST_LOOKUP[sawWave[(int) (nextSample >> FRQ_BITS)]];
                    nextSample = (nextSample + frq) % WAVELEN;
                }
            } else {
                // SYNCH
        ...
        case PULSE:
            if (!sync) {
                for (int i = 0; i < GENLEN; i++) {
                    ...
                    intBuffer[bIndex++] = pulseWave[pwid + (int) (nextSample >> FRQ_BITS)];
                    nextSample = (nextSample + frq) % WAVELEN;
                }
            } else {
                for (int i = 0; i < GENLEN; i++) {
        ...
        case PULSE | SAW | TRIANGLE:
            ...
            if (!sync) {
                for (int i = 0; i < GENLEN; i++) {
                    ...
                    intBuffer[bIndex++] = sam1 != 0 ? lookup[lookupW[(int) (nextSample >> FRQ_BITS)]] : 0;
                    nextSample = (nextSample + frq) % WAVELEN;
                }
            } else {
                for (int i = 0; i < GENLEN; i++) {
        ...
        case NOISE | PULSE | TRIANGLE | SAW:
            ...
            for (int i = 0; i < GENLEN; i++) {
                ...
                int loops = 0;
                while (delay < 0) {
                    int bit0 = (int) ((noise_reg >> 22) ^ (noise_reg >> 17)) & 0x1;
                    noise_reg <<= 1;
                    noise_reg &= 0x7fffff;
                    noise_reg |= bit0;

                    if (loops < 4) {
                        newNoiseData += (int) (((noise_reg & 0x400000L) >> 11) | ((noise_reg & 0x100000L) >> 10)
                                | ((noise_reg & 0x010000L) >> 7) | ((noise_reg & 0x002000L) >> 5)
                                | ((noise_reg & 0x000800L) >> 4) | ((noise_reg & 0x000080L) >> 1)
                                | ((noise_reg & 0x000010L) << 1) | ((noise_reg & 0x000004L) << 2));
                        loops++;
                    }
                    delay += waveLen / 32;
                }
