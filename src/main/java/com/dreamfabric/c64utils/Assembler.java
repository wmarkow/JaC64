...
/**
 * A simple assembler for JaC64 (C64) / 6502 Joakim Eriksson, JaC64.com /
 * Dreamfabric.com ---------------------------------------------- Should be
 * almost compatible (?) with a65 and some other C64 assemblers
 * ---------------------------------------------- To get full functionality for
 * loading binaries you need to override openBinary!
 *
...
public class Assembler {
    ...
    public int[] assemble(String s, int start) {
        ...
        for (int i = 0, n = lines.length; i < n; i++) {
            currentLine = lines[i];
            if (assembleLine(lines[i], memory, pos) == 1)
                break;
            lineNo++;
        }
    ...
    private void loadBinary(String binary) {
        try {
            ...
            System.out.println("Loaded binary file at: " + hex4(initPos) + " len: " + (pos - initPos));
            fs.close();
        } catch (Exception e) {
    ...
    private int resolve(String name, int pos) {
        ...
        for (int i = 0, n = name.length(); i < n; i++) {
            c = name.charAt(i);
            if (c == '+') {
                return resolve(name.substring(0, i), pos) + resolve(name.substring(i + 1), pos);
            } else if (c == '-') {
                return resolve(name.substring(0, i), pos) - resolve(name.substring(i + 1), pos);
            } else if (c == '/') {
                return resolve(name.substring(0, i), pos) / resolve(name.substring(i + 1), pos);
    ...
    private void setRelValue(int pos, int val) {
        ...
        if (DEBUG_LEVEL > 0) {
            System.out.println("Set branch value of " + memory[pos] + " at " + hex4(pos));
        }
    }
    ...
    private void createRef(int type, int pos, String s) {
        ...
        if (DEBUG_LEVEL > 0) {
            System.out.println("*** Creating reference to: '" + s + "' at " + hex4(pos));
        }
    ...
    private boolean addLabel(String label, String op, String operand) {
        ...
        if ("*".equals(label)) {
            ...
            if (DEBUG_LEVEL > 0) {
                System.out.println("*** Added label: '" + label + "' = " + hex4(lpos[0]));
            }
            return rval;
        }
    }
