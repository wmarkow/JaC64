...
public class SID {
    ...
    /**
     * This is a patch against ReSID engine in libsidplay2-2.1.1 to make its sound
     * closer to 6581R4 chip.<BR>
     * 
     * http://bel.fi/~alankila/c64-sw/
     * 
     * <P>
     * <B>Good simulation of strong lead and bass distortion.</B> Tel's leads,
     * Jeff's bass, Mechanicus's extreme distortion: it all seems to reproduce quite
     * nicely.
     * 
     * <P>
     * <B>Good simulation of advanced effects peculiar to 6581</B>, especially on
     * Jeff's songs that target 6581R4.
     * 
     * <P>
     * <B>Parameters are now in sidplay2.ini file.</B> No recompile is necessary
     * when testing alternative parameters.
     * 
     * <P>
     * <B>Bugs in some songs, especially related to lowpass filter output.</B> The
     * middle part of Spaceman Salutes Commodore and intro for Snake Disco are
     * incorrectly rendered. (ReSID's linear filter is totally wrong with them too.)
     * Vendetta probably gets too much edge in the bass, too. It seems that some
     * kind of additional low-pass filtering needs to happen during so far unknown
     * filter conditions.
     * <P>
     * <UL>
     * <LI>8.12.2007 Separate parameters for band-pass (high) and low-pass (low)
     * distortion. This doubles the number of tunables, but seems like a necessary
     * step to open experimentation on this front.
     * <LI>6.12.2007 New distortion algorithm. After I scratched the whole model of
     * substracting filter states from each other. The new model simply calculates
     * distortion by mixing in the filter state terms and input in linear
     * combination. The following rules seem to apply:
     * <OL>
     * <LI>Vlp term must be about 0.9. More than that, and Jeff's bass sounds lose
     * their edge. Too little, and pretty much everything distorts wrong.
     * <LI>Vbp term be something negative like -0.5 or -1 or Vendetta and Meanwhile
     * The Planet doesn't work.
     ...
     * samplings, too. Therefore it makes for more complicated distorted sounds, and
     * better replicates the effects in f.e. Mechanicus.
     ...
     * songs.
     * <LI>4.12.2007 Made tunables out of the rest of the distortion parameters,
     * save from one that I just added: low-pass filtering for my crude w0_eff term.
     * This seems to reduce the artifacts on Robocop 3, and generally doesn't seem
     * to harm the songs.
     * <LI>3.12.2007 A new distortion algorithm. A lot of time has passed. I made
     * some improvements that force more distortion out of the model. I feel that
     * this reinstates some of my LEADDIST effects and should be closer to correct
     * model. I'm reasonably happy about how it turned out. This is definitely the
     * best all-around emulation to date.
     * 
     ...
     * backwards. Maybe.
     * <LI>13.2.2007 Defeat with the masterful TBB songs such as 0_N_0 and Meanwhile
     * The Planet. The 0_N_0 makes a lot of snapping because in a real chip the
     * distortion terms are "filtered" by the other filter components, while mine
     * are filtered by a static ad-hoc estimate. So, the snapping here comes from
     * the LEADDIST term and it doesn't make the right kind of effect.
     * 
     * Meanwhile The Planet makes masterfully precise use of the filter, and there
     * isn't enough distortion for ReSID rendition to be palatable. Thanks to Grue
     * for digging these songs up. Also fixed invalid sidplay2.ini configuration
     * file (was not using 6581R4 curve). This, too, was spotted by Grue.
     * 
     ...
     * and discovered that level of lp and hp outputs dropped dramatically when bp
     * was driven to distortion. I observed loss of up to 20 dB. I probably have to
     * make many more measurements to allow mapping an estimate for this behaviour.
     * <LI>12.2.2007 I added a minor tweak, adding an estimate of the resonance term
     * in the op-amp loading measurement. I also made the bass distortion slightly
     * more prominent, although it still isn't so harsh as the real chip produces.
     * <LI>11.2.2007 I decided the price for fixing RoboCop 3 on 10.2. changes was
     * too high to pay. It put a few dB of distortion back and migrated the
     * distortion back to later time. Other changes in frequency drift
     * implementation however helped a lot even with RoboCop 3, although I do think
     * it still distorts too much. I think overall I'm now better off.
     * 
     ...
     * improved locutox's cover of Purple Shades. (Not included on sample set.) I
     * also set bass level to -4.4 dB, because it comes about 1.5 dB louder than
     * treble or bandpass. This is in full contrast to comments in ReSID that claim
     * no level difference exists. Strange, but I can see it plain as day on FFT.
     ...
     * 1.234/L^2, while the error in non-interpolated lookup is bounded by 0.7854/L
     * + 0.4113/L^2, see
    ...
    public SID() {
        ...
        set_sampling_parameters(985248, sampling_method.SAMPLE_FAST, 44100, -1, 0.97);
    ...
    public void set_distortion_properties(int Lt, int Ls, int Ll, int Lb, int Lh, int Ht, int Hs, int Hl, int Hb,
            int Hh) {
    ...
    public State read_state() {
        ...
        for (i = 0, j = 0; i < 3; i++, j += 7) {
            ...
            state.sid_register[j + 4] = (char) ((wave.waveform << 4) | ((wave.test != 0) ? 0x08 : 0)
                    | ((wave.ring_mod != 0) ? 0x04 : 0) | ((wave.sync != 0) ? 0x02 : 0)
                    | ((envelope.gate != 0) ? 0x01 : 0));
     ...
     * function is originally from resample-1.5/filterkit.c by J. O. Smith.
     * 
     * @param x
     * @return
     */
     ...
     * @return
     */
    public boolean set_sampling_parameters(double clock_freq, sampling_method method, double sample_freq,
        ...
        cycles_per_sample = (int /* cycle_count */) (clock_freq / sample_freq * (1 << FIXP_SHIFT) + 0.5);
        ...
        int res = method == sampling_method.SAMPLE_RESAMPLE_INTERPOLATE ? FIR_RES_INTERPOLATE : FIR_RES_FAST;
        int n = (int) Math.ceil(Math.log(res / f_cycles_per_sample) / Math.log((double) 2));
        fir_RES = 1 << n;
     ...
     * <P>
     * In some applications, e.g. a C64 emulator, it can be desirable to synchronize
     * sound with a timer source. This is supported by adjustment of the SID
     * sampling frequency.
     * <P>
     ...
     * that any adjustment of the sampling frequency will change the characteristics
     * of the resampling filter, since the filter is not rebuilt.
     * 
     * @param sample_freq
     */
    public void adjust_sampling_frequency(double sample_freq) {
        cycles_per_sample = (int /* cycle_count */) (clock_frequency / sample_freq * (1 << FIXP_SHIFT) + 0.5);
    }
    ...
    public void clock(int /* cycle_count */ delta_t) {
        ...
        while (delta_t_osc != 0) {
            ...
            for (i = 0; i < 3; i++) {
                ...
                int /* reg24 */ delta_accumulator = ((accumulator & 0x800000) != 0 ? 0x1000000 : 0x800000)
                        - accumulator;
    ...
    protected int clock_fast(CycleCount delta_t, short buf[], int n, int interleave) {
        ...
        for (;;) {
            int /* cycle_count */ next_sample_offset = sample_offset + cycles_per_sample + (1 << (FIXP_SHIFT - 1));
     ...
     * interpolation.
     * <P>
     * Here the chip is clocked every cycle. This yields higher quality sound since
     * the samples are linearly interpolated, and since the external filter
     * attenuates frequencies above 16kHz, thus reducing sampling noise.
     * 
     * @return
     */
    protected int clock_interpolate(CycleCount delta_t, short buf[], int n, int interleave) {
        ...
        for (;;) {
            int /* cycle_count */ next_sample_offset = sample_offset + cycles_per_sample;
            int /* cycle_count */ delta_t_sample = next_sample_offset >> FIXP_SHIFT;
            if (delta_t_sample > delta_t.delta_t) {
                break;
            }
     ...
     * frequency, this implementation dramatically reduces the computational effort
     * in the filter convolutions, without any loss of accuracy. The filter
     * convolutions are also vectorizable on current hardware.
     * <P>
     * Further possible optimizations are: * An equiripple filter design could yield
     * a lower filter order, see
     * http://www.mwrf.com/Articles/ArticleID/7229/7229.html * The Convolution
     * Theorem could be used to bring the complexity of convolution down from O(n*n)
     * to O(n*log(n)) using the Fast Fourier Transform, see
     * http://en.wikipedia.org/wiki/Convolution_theorem * Simply resampling in two
     * steps can also yield computational savings, since the transition band will be
     * wider in the first step and the required filter order is thus lower in this
     * step. Laurent Ganier has found the optimal intermediate sampling frequency to
     * be (via derivation of sum of two steps): 2 * pass_freq + sqrt [ 2 * pass_freq
     * * orig_sample_freq * (dest_sample_freq - 2 * pass_freq) / dest_sample_freq ]
     * <P>
     * 
     * @return
     */
