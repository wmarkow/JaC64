...
public class WaveformGenerator implements IWave6581, IWave8580 {
    ...
    public void writeCONTROL_REG(int /* reg8 */ control) {
        ...
        if (ANTTI_LANKILA_PATCH) {
            /*
             * SounDemoN found out that test bit can be used to control the noise register.
             * Hear the result in Bojojoing.sid.
             */
            ...
            if (waveform > 8) {
                shift_register &= 0x7fffff ^ (1 << 22) ^ (1 << 20) ^ (1 << 16) ^ (1 << 13) ^ (1 << 11) ^ (1 << 7)
                        ^ (1 << 4) ^ (1 << 2);
            }
        } else {
            // Test bit set.
    ...
    /**
     * Triangle: The upper 12 bits of the accumulator are used. The MSB is used to
     * create the falling edge of the triangle by inverting the lower 11 bits. The
     * MSB is thrown away and the lower 11 bits are left-shifted (half the
     * resolution, full amplitude). Ring modulation substitutes the MSB with MSB EOR
     * sync_source MSB.
     * 
     * @return triangle
     */
    protected int /* reg12 */ output___T() {
        int /* reg24 */ msb = ((ring_mod != 0) ? accumulator ^ sync_source.accumulator : accumulator) & 0x800000;
    ...
    /**
     * Pulse: The upper 12 bits of the accumulator are used. These bits are compared
     * to the pulse width register by a 12 bit digital comparator; output is either
     * all one or all zero bits.
     * <P>
     * NB! The output is actually delayed one cycle after the compare. This is not
     * modeled.
     * <P>
    ...
    protected int /* reg12 */ outputN___() {
        return ((shift_register & 0x400000) >> 11) | ((shift_register & 0x100000) >> 10)
                | ((shift_register & 0x010000) >> 7) | ((shift_register & 0x002000) >> 5)
                | ((shift_register & 0x000800) >> 4) | ((shift_register & 0x000080) >> 1)
                | ((shift_register & 0x000010) << 1) | ((shift_register & 0x000004) << 2);
    }

    /**
     * Combined waveforms: By combining waveforms, the bits of each waveform are
     * effectively short circuited. A zero bit in one waveform will result in a zero
     * output bit (thus the infamous claim that the waveforms are AND'ed). However,
     * a zero bit in one waveform will also affect the neighboring bits in the
     * output. The reason for this has not been determined.
     * <P>
     * Example:
     * 
     * <pre>
     ...
     * this case does not act as a digital state machine. Tests show that minor (1
     * bit) differences can actually occur in the output from otherwise identical
     * samples from OSC3 when waveforms are combined. To further complicate the
     * situation the output changes slightly with time (more neighboring bits are
     * successively set) when the 12-bit waveform registers are kept unchanged.
     * <P>
     * It is probably possible to come up with a valid model for the behavior,
     * however this would be far too slow for practical use since it would have to
     * be based on the mutual influence of individual bits.
     * <P>
     * The output is instead approximated by using the upper bits of the accumulator
     * as an index to look up the combined output in a table containing actual
     * combined waveform samples from OSC3. These samples are 8 bit, so 4 bits of
     * waveform resolution is lost. All OSC3 samples are taken with FREQ=0x1000,
     * adding a 1 to the upper 12 bits of the accumulator each cycle for a sample
     * period of 4096 cycles.
     * <P>
     * Sawtooth+Triangle: The sawtooth output is used to look up an OSC3 sample.
     * <P>
     * Pulse+Triangle: The triangle output is right-shifted and used to look up an
     * OSC3 sample. The sample is output if the pulse output is on. The reason for
     * using the triangle output as the index is to handle ring modulation. Only the
     * first half of the sample is used, which should be OK since the triangle
     * waveform has half the resolution of the accumulator.
     * <P>
     * Pulse+Sawtooth: The sawtooth output is used to look up an OSC3 sample. The
     * sample is output if the pulse output is on.
     * <P>
    ...
    /**
     * Combined waveforms including noise: All waveform combinations including noise
     * output zero after a few cycles.
     * <P>
     * NB! The effects of such combinations are not fully explored. It is claimed
     * that the shift register may be filled with zeroes and locked up, which seems
     * to be true.
     * <P>
     * We have not attempted to model this behavior, suffice to say that there is
     * very little audible output from waveform combinations including noise. We
     * hope that nobody is actually using it.
     */
