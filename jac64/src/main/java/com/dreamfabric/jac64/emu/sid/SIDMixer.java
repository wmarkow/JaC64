...
public class SIDMixer {
    ...
    private String[] efxNames = { "EFX_NONE", "EFX_FLANGER_1", "EFX_FLANGER_2", "EFX_FLANGER_3", "EFX_PHASER_1",
            "EFX_PHASER_2", "EFX_CHORUS_1", "EFX_CHORUS_2", "EFX_ECHO_1", "EFX_ECHO_2", "EFX_REV_SMALL", "EFX_REV_MED",
            "EFX_REV_LARGE", "EFX_FSWEEP", "EFX_FSWEEP_RES", };
    ...
    public void init(SIDVoice6581[] channels, SIDVoice sample, AudioDriver driver) {
        ...
        for (int i = 0, n = LFO_WAVELEN; i < n; i++) {
            sine10Hz[i] = (int) (500 + (500 * Math.sin(i * 2 * 3.1415 / LFO_WAVELEN)));
        }
    ...
    public void setFilterCtrl(int data) {
        ...
        if (DEBUG) {
            System.out.println(
                    "Filter: " + data + " => " + (lpOn ? "LP " : "") + (bpOn ? "BP " : "") + (hpOn ? "HP " : ""));
        }
        recalcFilter();
    }
        ...
        if (soundOn) {
            ...
            for (int i = 0, n = SIDVoice6581.GENLEN; i < n; i++) {
                ...
                if (filterOn > 0) {
                    ...
                    inval = (int) ((bpOn ? vbp : 0) + (hpOn ? vhp : 0) + (lpOn ? vlp : 0));
                }
