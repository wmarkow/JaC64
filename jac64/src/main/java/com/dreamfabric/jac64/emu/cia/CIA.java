...
public class CIA {
    ...
    private class CIATimer {
        ...
        private void loadTimer(long cycles) {
            ...
            if (TIMER_DEBUG && offset == 0x10d00) {
                System.out.println(ciaID() + ": " + id + " - timer loaded at " + cycles + " with: " + latch + " diff "
                        + (cycles - lastLatch));
                lastLatch = cycles;
            }
        }
        ...
        private void triggerInterrupt(long cycles) {
            if (TIMER_DEBUG && offset == 0x10d00)
                System.out.println(ciaID() + ": " + id + " - trigger interrupt at: " + cycles + " nextZero: " + nextZero
        ...
        public void doUpdate(long cycles) {
            ...
            if (cycles == nextUpdate) {
                ...
                while (cycles >= nextUpdate) {
                    System.out.println(ciaID() + ": " + id + " ** update at: " + cycles + " expected: " + nextUpdate
                            + " state: " + state);
                    update(nextUpdate);
                }
            }
